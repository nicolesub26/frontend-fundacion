...
export class VoluntarioComponent implements OnInit {
  ...
  toggleEstado(item: Voluntario) {
    ...
    const ok = confirm(`¿Seguro que deseas ${accion} a ${item.nombre}?`);
    if (!ok) return;

    // Usar los endpoints correctos: PUT /api/voluntarios/{id}/activar o /desactivar
    const request$ = activar
      ? this.service.activar(item.idVoluntario)
      : this.service.desactivar(item.idVoluntario);

    request$.subscribe({
      next: (res) => {
        // 1. Actualizar el voluntario en la lista local
        const idx = this.data.findIndex(v => v.idVoluntario === res.idVoluntario);
        if (idx > -1) {
          this.data[idx] = res;
        }

        // 2. Aplicar filtros para actualizar la vista
        this.applyFilters();
        ...
        const mensaje = `Voluntario ${accion === 'activar' ? 'activado' : 'desactivado'} exitosamente`;
        this.notify('success', mensaje);
      },
      error: (err) => {
        this.notify('error', this.extractError(err, `No se pudo ${accion} el voluntario`));
        console.error(err);
      }
    });
  }
